...
import hashlib, json, pathlib, subprocess, xml.etree.ElementTree, yaml, difflib
...
class State:
    ...
    def clang_tidy_run(self,
        ...
        ex = execute(['clang-tidy', '-p=' + COMPILE_COMMAND_JSON_FILENAME, '-quiet', '-header-filter=.*',
                      '-export-fixes={}'.format(CLANG_TIDY_YAML_FILENAME)] +  [executable_filename] +
                      implementation_file_names if implementation_file_names is not None else [],
                     timeout=timeout)
